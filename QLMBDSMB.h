...
void checkEventPageQLMB_DSMB(short **mapID, Shape *shape, Input *input, Button *mainButton, Board &BOARDMB,Button *pre_next);
...
void drawPageQLMB_DSMB(short **mapID,Shape *shape);
...
void checkEventPageQLMB_DSMB(short **mapID, Shape *shape, Input *input, Button *mainButton, Board &boardMB,Button *pre_next)
{
    ...
    while (true)
    {
        ...
        if (ismouseclick(WM_LBUTTONDOWN))
        {
		    ...
		    if((ID>=ID_BUTTON_PAGE_MAIN && ID<=ID_BUTTON_PAGE4 && ID != ID_BUTTON_PAGE_QLMB) || ID == ID_BUTTON_THONGKE_1){            
		    	clearDSMB(dsMB);
		    	clearDSCB(dsCB);
            	return;
            }             
...
void drawPageQLMB_DSMB(short **mapID, Shape *shape){
    ...
    Input input[3] = {
	{450, 570, 15, 190,0 }, //so hieu 
	{890, 570, 4, 80, 0},  //loai 
	{450, 620, 40, 520, 0} //so cho
	};
    for (int i = 0; i < 3; i++)
    {	
    ...
    checkEventPageQLMB_DSMB(mapID, shape, input, mainButton, boardMB,pre_next);
    	for(int i = 0;i<3;i++)
    		delete[] input[i].s;
}
void getInputPageQLMB(Input &input, short &inputID, bool &isEnter)
